...
public class UserDbStorage extends BaseDbStorage<User> implements UserStorage {
    ...
    public void addFriend(long userId, long friendId) {
        ...
        List<Integer> reverseStatus = jdbc.query(CHECK_FRIEND_QUERY,
                (rs, rowNum) -> rs.getInt("friendship_status"), friendId, userId);

        if (reverseStatus.isEmpty()) {
            jdbc.update(INSERT_FRIEND_QUERY, userId, friendId, 0);
        } else {
            jdbc.update(INSERT_FRIEND_QUERY, userId, friendId, 1);
            jdbc.update(UPDATE_FRIEND_STATUS_QUERY, 1, friendId, userId);
        }
    }
